	...
	class Vista {
		...
		public function dibujarReloj($modelo){
			...
			if($modelo->seg>=15 && $modelo->seg<61){
				imageline($this->img, $modelo->centrox, $modelo->centroy, $modelo->arrptosint[$modelo->seg-15]->x, $modelo->arrptosint[$modelo->seg-15]->y, $this->rojo);											
			}
			if($modelo->seg>0 && $modelo->seg<15){
				imageline($this->img, $modelo->centrox, $modelo->centroy, $modelo->arrptosint[$modelo->seg+45]->x, $modelo->arrptosint[$modelo->seg+45]->y, $this->rojo);											
			}
			...
			if($modelo->hora>2 && $modelo->hora<15){
				...
				imagesetthickness($this->img, 3);
				imageline($this->img, $modelo->centrox, $modelo->centroy, $modelo->arrptosint[$modelo->hora*5+45]->x, $modelo->arrptosint[$modelo->hora*5+45]->y, $this->negro);
			}
